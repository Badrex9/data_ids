...
def train_model(X,Y, model_old, epochs=20):
    ...
    model.save('./Y_prediction/model_2D_cnn_retrain_6.h5')
...
X_input = np.load('./X_input_split_train_n0_smote_6/X_input_0.npy')
d_model = np.shape(X_input)[1]
for i in range(1, 20):
    X_input = np.concatenate((X_input, np.load('./X_input_split_train_n0_smote_6/X_input_'+str(i)+'.npy')))
Y = np.load('./X_input_split_train/Y_n0_smote_6.npy')
