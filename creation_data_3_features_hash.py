...
def hash_ip_port(ip1, ip2, port2):
    min_ip = min(ip1, ip2)
    max_ip = max(ip1, ip2)
    #min_port = min(port1, port2)
    #max_port = max(port1, port2)
    hash_input = f"{min_ip}{max_ip}{port2}"
...
def transfo(X, source_ip, dest_ip, dest_port):
    ...
    for raw in tqdm(X):
        ...
        if index != -1:
            new_mat = np.insert(flows[index],0, raw, axis=1)[:, :10]
            flows[index] = new_mat  #décalage
            data_input.append(new_mat) #Ajout à l'input
        else:
            new_mat = np.tile(raw, (10, 1)).T  #Créer une matrice avec 20 fois le même vecteur
...
split_npy_save(X_input, 20, 'X_input_split_train_n2')
np.save('./X_input_split_train_n2/Y.npy', Y)