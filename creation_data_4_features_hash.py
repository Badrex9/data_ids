...
def hash_ip_port(ip1, ip2, port1, port2):
    min_ip = min(ip1, ip2)
    max_ip = max(ip1, ip2)
    min_port = min(port1, port2)
    max_port = max(port1, port2)
    hash_input = f"{min_ip}{max_ip}{min_port}{max_port}"
...
def transfo(X, source_ip, dest_ip, source_port, dest_port):
    ...
    for raw in tqdm(X):
        ...
        nouveau_couple = hash_ip_port(sip, dip, sp, dp)
        
        index = binary_search_hash(hashes, nouveau_couple)
        if index != -1:
            new_mat = np.insert(flows[index],0, raw, axis=1)[:, :20]
