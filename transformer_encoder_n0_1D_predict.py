...
for j in tqdm(range(0, len_without_rest, batch_size), desc=f"Predict {1}", leave=False):
    x = X_test[j:j+batch_size].view(batch_size, d_model, 1).transpose(1,2).to(device)
    value = model(x)
    if (j==0):
...
if reste!=0:
    x = X_test[j:j+reste].view(reste, d_model, 1).transpose(1,2).to(device)
    value = model(x)
    if (j==0):
        output = torch.argmax(F.softmax(value), dim=1)
    else: 
...
np.save("./Y_prediction/y_pred_transformer_2D_smote.npy", output)