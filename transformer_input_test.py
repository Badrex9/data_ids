...
class MyViT(nn.Module):
  ...
  def __init__(self, batch_size, d_pacquet, d_model, d_historique, out_d, n_heads, n_blocks, d_ff):
    ...
    self.class_token = nn.Parameter(torch.rand(1, self.d_model)) #.double()
  ...
  def forward(self, images):
    ...
    #print("Tokens concatenés size: ", tokens.size())
    # Adding positional embedding
    out = tokens + self.positional_embeddings.repeat(self.batch_size, 1, 1)

    #print("Positional encoding: ", out)
    # Transformer Blocks
...
d_historique = np.shape(X_input)[2] #np.shape(X_input)[1] #Longueur du vecteur d'entrée (d_model) normalement 82
num_heads = 1  #d_model % num_heads == 0, "d_model must be divisible by num_heads"
num_layers = 2 #RTIDS Nombre de répétition des encoders/decoders
d_ff = 1024 #RTIDS dimension du FFN layer
dropout = 0.5 #RTIDS
batch_size = 128 #RTIDS batch_size = 128
PATH = "./modele_transformer_2D_test.pth"
...
for epoch in range(N_EPOCHS):#, desc="Training"):
    ...
    for j in tqdm(range(0, len_without_rest, batch_size), desc=f"Epoch {epoch + 1} in training", leave=False):
        ...
        if epoch==74:
            print("Argmax: ", torch.topk(F.softmax(y_hat), 1).indices.view(batch_size))
