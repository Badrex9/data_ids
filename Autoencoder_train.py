...
def train_model(X,Y, model_old, epochs=20):
    ...
    model_old.save('../Y_prediction/model_autoencoder_' +str(Y_label)+ '_' + str(Nb_sample)+ '.h5')
...
if Nb_sample==1000:
    X_autoencoder = np.load('./generate/X_generate_' + str(Y_label) +'.npy')
else:
    X_autoencoder = np.load('./generate/X_generate_' +str(Y_label)+ '_' + str(Nb_sample)+'.npy')
Y_autoencoder = [Y_label for i in range(Nb_sample)]
